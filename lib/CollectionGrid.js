...
export class CollectionGrid extends React.Component {
  constructor(props, context) {
    super(props, context);
    const { store, rowHeight = 32, actions, keyBindings, maxRows = 50 } = props;
    this.store = store;
  ...
  set scrollTop(n) {
    this._scrollTop = Math.max(
      0,
      Math.min(n, this.rows.length - this.scrollHeight)
    );
  }
  ...
  get realScrollTop() {
    return (
      Math.floor(this.scrollTop / this.overScan) * this.overScan -
      (this.overScan >> 1)
    );
  }
  @computed
  ...
  render() {
    return (
      <div
        className={"ZA Grid " + (this.props.className || "")}
        style={{
          display: "flex",
          flexFlow: "column",
          ...this.props.style
      ...
      >
        ...
        >
          ...
          >
            {[...Array(this.realScrollHeight)].map((_, index) => (
              <GridRow key={index} grid={this} index={index} />
            ))}
          </div>
        </div>
        <StatusBar grid={this} />
  ...
  actions = {
    ...
    SELECT_ROW: () =>
      ...
      this.currentRow &&
      this.currentRow.toggleSelected(),
    ...
    MOVE_NODE_IN: () => {
      ...
      else if (!this.store.currentRow.isLeaf)
        this.store.currentRow = this.store.currentRow.children[0];
    }
  };
  ...
  handleKeyDown = event => {
    ...
    key +=
      event.key === " "
        ? "Space"
        : event.key[0].toUpperCase() + event.key.substr(1);
  ...
  refScrollContainer = el => {
    ...
    this.observer = new window.ResizeObserver(entries => {
      this.scrollHeight = Math.min(
        this.maxRows,
        Math.floor(el.clientHeight / this.rowHeight)
      );
    });
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{
          marginTop:
            grid.rowHeight * (grid.realScrollTop - grid.scrollTop) + "px"
        }}
      />
    );
  }
...
export class StatusBar extends React.Component {
  ...
  toggleFilter = event => {
    if (event.button === 0) {
      this.props.grid.store.filterEnabled = !this.props.grid.store
        .filterEnabled;
    ...
    return (
      <div className="statusbar" style={{ position: "relative", zIndex: 2 }}>
        ...
        <span className="toolbar">
          <span
            className={"filter button " + (this.filterOn ? "on" : "off")}
            onMouseDown={this.toggleFilter}
          />
          <span
            tabIndex="-1"
              ...
              >
                {store.columns.map((column, index) => (
                  <div
                    className="item"
                    key={index}
                    onMouseDown={event => this.toggleHideColumn(event, column)}
                  >
                    <span
                      className={
                        "checkbox " +
                        (this.isColumnHidden(column) ? "" : " selected")
                      }
                    />
                    {column.title}
                  </div>
