...
export class ColumnStore {
  ...
  constructor({ store, fields, labelKey, columnOrder }) {
    ...
    this.labelColumn = new Column({
      store: this,
      field: this.fields[this.labelKey]
    });
    this.columnOrder =
      columnOrder || Object.keys(this.fields).filter(key => key !== labelKey);
  }

  @computed
  get columns() {
    return this.columnOrder.map(
      key => new Column({ store: this, field: this.fields[key] })
    );
  }
  ...
  filterRow(row) {
    return (
      this.labelColumn.filterRow(row) &&
      this.columns.every(column => column.filterRow(row))
    );
  }
  filterRows(rows) {
    return this.columns.reduce(
      (rows, column) => column.filterRows(rows),
      this.labelColumn.filterRows(rows)
    );
  }
}
...
export class Column {
  ...
  getter = row => {
    return this.field.getter
      ? this.field.getter(row.record, this.field.key)
      : dot.get(row.record, this.field.key);
  };

  value = row => {
    return this.field.value
      ? this.field.value(this.getter(row))
      : this.getter(row);
  };

  format = row => {
    return this.field.formatter
      ? this.field.formatter(this.value(row), row, this.field.key)
      : this.value(row);
  };
