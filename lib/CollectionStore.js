...
export class CollectionStore {
  ...
  constructor({
    ...
    if (loadRecords) {
      this._loadRecords = loadRecords;
      this.loadRecords();
    }
  }

  loadRecords = () => {
    if (!this._loadRecords) return;
    Promise.resolve(this._loadRecords()).then(
      action(records => (this.records = records))
    );
  };
